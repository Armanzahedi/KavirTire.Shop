...
using KavirTire.Shop.Infrastructure.Payment.PaymentGateway.SamanKish.Models;
using KavirTire.Shop.Infrastructure.PaymentIFBinding;
using Newtonsoft.Json;
using RestSharp;
using GetTokenResponse = KavirTire.Shop.Infrastructure.Payment.PaymentGateway.SamanKish.Models.GetTokenResponse;

namespace KavirTire.Shop.Infrastructure.Payment.PaymentGateway.SamanKish;

[BankType(Bank.Saman)]
public class SamanKishPaymentGateway : Application.Payments.Services.PaymentGateway.PaymentGateway
{
    private readonly Ipg _ipg;
    private readonly IRepository<Domain.Payments.Payment> _paymentRepo;

    public SamanKishPaymentGateway(Ipg ipg, IRepository<Domain.Payments.Payment> paymentRepo) : base(ipg)
    {
        _ipg = ipg;
    ...
    public override Task<string> GetGatewayUrl(Invoice invoice,  Domain.Payments.Payment payment,BankAccount bankAccount, BankAccount? postBankAccount, string? customerMobileNumber = "09921744820")
    {
        ...
        //     {
        //         Amount = Convert.ToInt64(invoice.TotalCost - invoice.TotalPostCost),
    ...
    public override async Task<VerifyTransactionResult> VerifyTransaction(Dictionary<string, string?> bankResponse)
    {
        ...
        if (bankResponseObj?.Status == SepResponseStatus.OK)
        {
            ...
            { TerminalId = _ipg.TerminalId, RefNum = bankResponseObj.RefNum, Password = _ipg.Password, Amount= bankResponseObj.Amount });
